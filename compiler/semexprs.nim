...
proc semConstrFields(c: PContext, info: TLineInfo, flags: TExprFlags,
  ...
  of nkRecList:
    for field in recNode:
      let status = semConstrFields(c, info, flags, nil, field, initExpr)
      mergeInitStatus(result.status, status.status)

    if t != nil and t.sons[0] != nil:
      var t = skipTypes(t.sons[0], skipPtrs)
      let status = semConstrFields(c, info, flags, t, t.n, initExpr)
      mergeInitStatus(result.status, status.status)
  ...
  of nkRecCase:
    ...
    for i in 1 .. <recNode.len:
      ...
      let s = semConstrFields(c, info, flags, nil, innerNode, initExpr)
...
proc semObjConstr(c: PContext, n: PNode, flags: TExprFlags): PNode =
  ...
  let initResult = semConstrFields(c, n.info, flags, t, t.n, n)
